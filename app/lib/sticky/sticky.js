angular.module("sticky", []).directive("sticky", function($window) {
    return {
        link: function(scope, element, attrs) {
            ...
            element = $(element);
            $win = $($win);
            if (scope.vm._stickyElements === undefined) {
                scope.vm._stickyElements = [];

                $win.bind("scroll.sticky", function(e) {
                    var pos = $win.scrollTop();
                    for (var i = 0; i < scope.vm._stickyElements.length; i++) {

                        var item = scope.vm._stickyElements[i];
            ...
            scope.vm._stickyElements.push(item);
